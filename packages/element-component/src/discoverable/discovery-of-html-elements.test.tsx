...
import { type Discover, discoverFor } from "./discovery-of-html-elements";
...
describe("discovery-of-html-elements", () => {
  describe("given multiple matches", () => {
    ...
    beforeEach(() => {
      const rendered = render(
        <div>
          ...
          <div data-some-other-attribute-test="some-value" data-some-third-attribute-test="10" />
          <div data-some-other-attribute-test="some-value" data-some-third-attribute-test="12" />
        </div>,
      );
    ...
    it("when querying single element, throws", () => {
      ...
      }).toThrow('Tried to discover single "[data-some-attribute-test]", but found multiple matches.');
    });
    ...
    it("when querying all elements, returns all elements having the attribute", () => {
      expect(discover.queryAllElements("some-attribute").attributeValues).toEqual([
        "some-first-value",
        "some-second-value",
      ]);
    });
    ...
    it("when querying all elements with same attribute value, returns all elements having the attribute value", () => {
      expect(discover.queryAllElements("some-attribute", "some-first-value").attributeValues).toEqual([
        "some-first-value",
      ]);
    });
    ...
    it("when querying all elements with the same attribute value, when getting a single element from that set, returns", () => {
      expect(
        discover.queryAllElements("some-other-attribute", "some-value").getSingleElement("some-third-attribute", "10")
          .discovered,
      ).toBeInTheDocument();
    });
  });
  ...
  describe("accessing attribute values", () => {
    describe("given there are elements with discoverable attribute values", () => {
      ...
      beforeEach(() => {
        const rendered = render(
          <div>
            <div data-some-attribute-test="some-first-value" data-some-different-test="other-value" />
      ...
      it("a discovered element has an attribute field", () => {
        expect(discover.getSingleElement("some-other-attribute").attributeValue).toBe("some-second-value");
      });

      it("a discovered element can discover attributes on the same element", () => {
        expect(discover.getSingleElement("some-attribute", "some-first-value").getAttribute("some-different")).toBe(
          "other-value",
        );
      });

      it("multiple discovered elements can discover attributes on the same elements", () => {
        expect(discover.queryAllElements("some-attribute").getAttributes("some-different")).toEqual([
          "other-value",
          null,
        ]);
      });
    ...
    describe("given there are elements with discoverable attribute values", () => {
      ...
      beforeEach(() => {
        ...
        const rendered = render(
          <div>
            <div data-some-attribute-test="some-first-value" data-some-different-test="other-value" />
            <div data-some-attribute-test="some-third-value" onClick={clickSpy} onContextMenu={rightClickSpy} />
            <div data-some-other-attribute-test="some-second-value" />
          </div>,
        );
      ...
      describe("when clicking", () => {
        beforeEach(async () => {
          await discover.getSingleElement("some-attribute", "some-third-value").click();
        });
      ...
      describe("when right clicking", () => {
        beforeEach(async () => {
          await discover.getSingleElement("some-attribute", "some-third-value").rightClick();
        });
      ...
      describe("when double clicking", () => {
        beforeEach(async () => {
          await discover.getSingleElement("some-attribute", "some-third-value").doubleClick();
        });
      ...
      it("clicking returns a promise (due to technical reasons)", () => {
        expect(discover.getSingleElement("some-attribute", "some-third-value").click()).toBeInstanceOf(Promise);
      });
    });
  });
});
