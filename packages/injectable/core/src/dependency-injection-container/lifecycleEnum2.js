  ...
  singleton: {
    ...
    getInstanceKey:
      ...
      (...instantiationParameters) => {
        if (instantiationParameters.length) {
          const injectableId = di.context.at(-1).injectable.id;

          throw new Error(
            `Tried to inject singleton "${injectableId}", but illegally to singletons, instantiationParameters were provided: "${instantiationParameters}".`,
          );
        }
